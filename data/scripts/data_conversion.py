"""
Converts given CiCMOD csv file into multiple xr.Datasets for each individual model. These will be stored under DESTINATION_DIR/climate_indices_``MODEL``.nc
...
def cicmod_csv_to_netcdf(data_url: PathLike, destination_dir: PathLike, file_name: str = "climate_indices") -> None:
    """
    Convert climate indices data from a CSV file to NetCDF format and save to the destination directory.
    It is made for converting the CiCMOD dataset csv results into netCDF.
    For each model present in the csv file a seperate file will be created:
    Example of csv file with two models "FOCI" and "CESM":
