...
from kalman_reconstruction.custom_plot import ncols_nrows_from_N, set_custom_rcParams, plot_state_with_probability, adjust_lightness
from kalman_reconstruction.statistics import normalize, compute_fft_spectrum, crosscorr
...
with open(results_path / "settings.yaml", "w") as stream:
    stream.write("#Settings used in the application of the Kalman_SEM on the CiCOD dataset.\n")
...
fig, axs = plt.subplots(nrows = len(state_variables), ncols =  1, layout = "constrained", figsize = (12,7), sharex=True)
axs = axs.flatten()
...
kalman_ccor = xr.merge(kalman_ccor_list)        # lagged cross correlation with all vars avaibales in the kalman results.

# %% STATE VARIABLES | Plot Lagged correlation of latent variable against state varibales
data_vars = state_variables
row_col = ncols_nrows_from_N(len(data_vars))

fig, axs = plt.subplots(layout = "constrained", figsize = (12,4), sharex=True, sharey=True, ncols = len(data_vars))
axs = axs.flatten()
...
fig.suptitle("CiCMOD | Absolute value of lagged correlation of latent variable against state varibales");
...
fig, axs = plt.subplots(layout = "constrained", figsize = (20,20), sharex=True, sharey=True, **row_col)
axs = axs.flatten()
...
fig.suptitle("CiCMOD | Absolute value lagged correlation of latent against climate indices.");
...
for idx, var in enumerate(data_vars):
    ax = axs[idx]
    xf, yf, yf_plot, f_min , f_max = compute_fft_spectrum(time = data.time, signal = data[var].values)
    ax.loglog(xf, yf_plot, label = "truth", alpha = 0.7)
    (xf, yf, yf_plot, f_min , f_max) = compute_fft_spectrum(time = kalman_states.time, signal = kalman_states[var].values)
